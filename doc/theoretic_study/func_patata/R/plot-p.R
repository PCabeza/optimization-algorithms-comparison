...
x <- seq(RX[1],RX[2],length=300) # default should be -9,9
y <- seq(RY[1],RY[2],length=300) #default -10,30
f <- function(x,y) {
    (y^2)/10 + cos((x^2 + y^2)/2)/4 -1/(x^2 + y^2 + 1); }
z <- outer(x,y,f)

persp(x,y,z,theta=theta,phi=phi,expand=0.5,col="lightblue"
      ,ltheta=120,lphi=90,shade=0.75,ticktype="detailed",border=NA)->res;
...
printsegment <- function(pO,pD,f,res,colO,colD) {
    ...
    cx <- seq(pO[1],pD[1],length=2);
    cy <- seq(pO[2],pD[2],length=2);
